...
class Api_model extends CI_Model {
    ...
    public function serviceRegister(){
        ...
        $data_price = array(
            ...
            "sp_first_day_price" => $this->input->post("sp_first_price"),
            "sp_first_day_start" => $this->input->post("sp_first_start"),
            "sp_first_day_end" => $this->input->post("sp_first_end"),
            "sp_first_month_price" => $this->input->post("sp_first_month_price"),
        ...
        if($this->input->post("sp_once_price_add") != ""){
            ...
            for($i = 0; $i < count($sp_once_price_add);$i++){
                ...
                $data_addoption_price = array(
                    ...
                    "sap_first_day_price" => $sap_first_price_add[$i],
                    "sap_first_day_start" => $sap_first_start_add[$i],
                    "sap_first_day_end" => $sap_first_end_add[$i],
                    "sap_first_month_price" => $sap_first_month_price_add[$i],
    ...
    public function serviceAddUpdate(){
        ...
        $new_data = array(
            ...
            "sva_input_price"=>$this->input->post("sva_input_price"),
    ...
    public function claimMake($mb_seq){
        // 마지막 청구 가져오기
        $pm_sv_seq = explode(",",$this->input->post("pm_sv_seq"));
        for($i = 0; $i < count($pm_sv_seq);$i++){
            $this->db->select("*");
            $this->db->from("payment a");
            $this->db->join("service b","a.pm_sv_seq = b.sv_seq","inner");
            $this->db->join("service_price c","b.sv_seq = c.svp_sv_seq","inner");
            $this->db->where("pm_sv_seq",$pm_sv_seq[$i]);
            ...
            if($query->num_rows() == 0){
                ...
                $this->db->where("sv_seq",$pm_sv_seq[$i]);
                $query = $this->db->get();
                $row = $query->row_array();
                ...
                $total_price = $row["svp_once_price"]-$row["svp_once_dis_price"]+$row["svp_month_price"]-$row["svp_month_dis_price"]-$row["svp_discount_price"]+$row["sp_first_day_price"];
                $payment_data = array(
                    "pm_type" => "1",
                    "pm_mb_seq" => $row["sv_mb_seq"],
                    "pm_sv_seq" => $pm_sv_seq[$i],
                    "pm_code" => $pm_code,
                    "pm_date" => $claim_date,
